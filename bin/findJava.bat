...
for /f %%w in ('echo \Wow6432Node\ ^& echo \') do (
  endlocal
  set "WW=%%w"
  setlocal enabledelayedexpansion

  call "!FJ_SCRIPT!" fun scan_reg_key "HKLM" "Software!WW!JavaSoft\Java Runtime Environment" <nul
  if "!ERRORLEVEL!" == "0" goto exit_ok

  call "!FJ_SCRIPT!" fun scan_reg_key "HKLM" "Software!WW!JavaSoft\Java Development Kit" <nul
  if "!ERRORLEVEL!" == "0" goto exit_ok
)
